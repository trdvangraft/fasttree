...
class Profile:
    def __init__(self, dna) -> None:
        ...
        self.__weight_profile = [1] * len(dna[0])
    ...
    def distance(self, profile: Profile):
        number_of_positions = len(self.dna[0])
        denom = sum([self.__weight_profile[i] * profile.__weight_profile[i] for i in range(number_of_positions)])
        top = sum([(1 - sum([a * b for a, b in zip(self.get_freq(i), profile.get_freq(i))])) * (self.__weight_profile[i] * profile.__weight_profile[i]) for i in range(number_of_positions)])        
        return (denom if denom > 0 else 0.01, top/denom if denom > 0 else 1)
